...
import { AuthView } from './components/AuthView'; // IMPORTANTE: Agregamos tu vista de Auth
...
function App() {
  ...
  const handleGenerate = async (notes: string, documentType: string, files: File[]) => {
    ...
    try {
      const response: SentenceResponse = await generateSentence({ judgeNotes: notes, caseFiles: files, role: currentRole, documentType: documentType });
      if (response.usageMetadata) updateTokens(response.usageMetadata);
      setState({ isLoading: false, error: null, result: response });
      setIsSplitView(false); 
    } catch (error: any) {
      setState(prev => ({ ...prev, isLoading: false, error: error.message || "Ocurrió un error inesperado." }));
    }
  };
  ...
  const startCanvasMode = (withAssistant: boolean, initialTopic: string = "") => {
      ...
      if (cleanTopic.toLowerCase().includes("abrir") || cleanTopic.toLowerCase().includes("lienzo")) cleanTopic = "";
      const initialText = cleanTopic ? `||| BORRADOR INICIAL: ${cleanTopic.toUpperCase()}\n\n...` : "||| NUEVO DOCUMENTO\n\nComienza a redactar tu proyecto jurídico...";
      setState({ isLoading: false, error: null, result: { text: initialText, matterDetected: LegalMatter.UNKNOWN, ratioDecidendi: "" } });
  ...
  const handleBlankFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
          ...
          setAppMode(AppMode.EDITOR);
          setState({ isLoading: true, error: null, result: null, loadingMessage: `Procesando ${file.name}...` });
          try {
              const { text, usageMetadata } = await digitizeDocument(file);
              updateTokens(usageMetadata);
              setCurrentFiles([file]);
              setActiveFileIndex(0);
              setState({ isLoading: false, error: null, result: { text: text, matterDetected: LegalMatter.UNKNOWN, ratioDecidendi: "" } });
              setIsSplitView(true);
              setInitialActiveTab('project');
          } catch (error: any) {
              setState(prev => ({ ...prev, isLoading: false, error: "No se pudo cargar el archivo: " + error.message }));
          } finally {
  ...
  const handleElevateFromStandalone = (fullDocument: string, files: File[]) => {
      setState({ isLoading: false, error: null, result: { text: fullDocument, matterDetected: LegalMatter.UNKNOWN, ratioDecidendi: "", source: 'AI' } });
      if (files.length > 0) { setCurrentFiles(files); setActiveFileIndex(0); }
      setAppMode(AppMode.EDITOR);
  ...
  return (
    <div className="h-screen flex flex-col bg-[#050505]">
      ...
      <main className="flex-1 flex overflow-hidden print:overflow-visible relative" onMouseMove={(e: any) => isResizing && resize(e)} onMouseUp={stopResizing}>
        
        {appMode === AppMode.LANDING && <LandingPage />}

        {appMode === AppMode.GENERATOR && (
            <>
                <div 
                    ref={sidebarRef}
                    className="h-full z-10 shadow-2xl shadow-black print:hidden relative flex-shrink-0"
                ...
                >
                    ...
                    <div 
                        ...
                        className="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-blue-500/50 transition-colors z-50 flex items-center justify-center group resizer-handle"
                    >
                        <div className="h-8 w-1 bg-slate-700 rounded group-hover:bg-blue-400"></div>
                    </div>
                </div>
                <div className="flex-1 h-full relative print:h-auto print:overflow-visible min-w-0">
                     <PreviewArea 
                        state={state} 
                        setState={setState} 
